...
class GetUserFromTokenTestCase(unittest.TestCase):
    ...
    def test_get_user_from_token_deactivated_user(self):
        ...
        user = CustomUser.objects.create_user(email='deactivated@example.com', password='password', is_active=False)
    ...
    def test_get_user_from_token_with_extra_spaces(self):
        # Create a mock request with extra spaces in the Authorization header
        request = self.factory.get('/')
        request.headers = {
            'Authorization': 'Token   valid_token_key'
        }

        # Mock the Token object retrieval
        mock_user = Mock()
        with patch('views.Token.objects.get', return_value=Mock(user=mock_user)) as mock_get:
            user = get_user_from_token(request)

        # Ensure the correct token key was used in the query
        mock_get.assert_called_once_with(key='valid_token_key')
    ...
    def test_get_user_from_token_case_insensitive_authorization_header(self):
        ...
        token = Token.objects.create(user=user, key='testtoken123')
...
class ForgotPasswordRequestOTPViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('forgot-password-request-otp')
...
class UpdateKYCViewTest(TestCase):
    ...
    def test_should_return_validation_errors_when_invalid_kyc_data_is_submitted(self):
        # Prepare invalid KYC data (e.g., missing required fields)
        invalid_kyc_data = {
        ...
        # Send a POST request with invalid KYC data
        response = self.client.post(reverse('update_kyc'), data=invalid_kyc_data, format='json')
    ...
    def test_update_kyc_serializer_failure(self):
        # Simulate invalid KYC data
        invalid_kyc_data = {
            'first_name': '',  # Assuming first_name is required and cannot be empty
            'last_name': 'Doe',
            'address': '123 Main St',
        }
        ...
        # Assert that the response contains the error message for the invalid field
        self.assertIn('first_name', response.data)
        self.assertEqual(response.data['first_name'], ['This field may not be blank.'])

    def test_update_kyc_missing_fields(self):
        # Remove required fields from the KYC data
        incomplete_kyc_data = {
            'first_name': '',  # Assuming first_name is required
            'last_name': '',  # Assuming last_name is required
            # 'address' is missing
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn('This field may not be blank.', response.data['first_name'])
        self.assertIn('This field may not be blank.', response.data['last_name'])
        self.assertIn('This field is required.', response.data['address'])

    def test_update_kyc_with_nested_objects(self):
        # Add nested KYC data
        nested_kyc_data = {
            'first_name': 'Jane',
            'last_name': 'Smith',
        ...
        # Mock the KYCSerializer to handle nested objects
        with patch('Api.views.KYCSerializer') as MockKYCSerializer:
            mock_serializer_instance = MockKYCSerializer.return_value
            mock_serializer_instance.is_valid.return_value = True
            mock_serializer_instance.data = nested_kyc_data
            mock_serializer_instance.save.return_value = None

            response = self.client.post(reverse('update-kyc'), nested_kyc_data, format='json')

            # Check that the serializer was called with the correct data
            MockKYCSerializer.assert_called_once_with(mock.ANY, data=nested_kyc_data, partial=True)
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertEqual(response.data['message'], 'KYC updated successfully')
            self.assertEqual(response.data['kyc'], nested_kyc_data)

    def test_concurrent_kyc_updates(self):
        # Create a second client to simulate concurrent access
        client2 = APIClient()
        client2.credentials(HTTP_AUTHORIZATION='Token ' + self.token.key)

        # Initial KYC data from the first client
        ...
        # Concurrent KYC data update from the second client
        ...
        # Fetch the latest KYC data to verify integrity
        kyc_record = KYC.objects.get(user=self.user)
        self.assertIn(kyc_record.first_name, ['Alice', 'Bob'])
        self.assertIn(kyc_record.last_name, ['Smith', 'Johnson'])
        self.assertIn(kyc_record.address, ['456 Elm St', '789 Pine St'])

    def test_partial_update_kyc_information(self):
        # Create initial KYC data for the user
        initial_kyc_data = {
            'first_name': 'John',
            'last_name': 'Doe',
            'address': '123 Main St',
        ...
        # New data to partially update the KYC information
        partial_kyc_data = {
            'address': '456 Elm St',
        }

        # Send a POST request to update KYC information
        ...
        # Assert that the KYC information is updated correctly
        self.assertEqual(updated_kyc.first_name, 'John')  # Ensure first name is unchanged
        self.assertEqual(updated_kyc.last_name, 'Doe')  # Ensure last name is unchanged
        self.assertEqual(updated_kyc.address, '456 Elm St')  # Ensure address is updated

        # Assert the response data
        self.assertEqual(response.data['message'], 'KYC updated successfully')
        self.assertEqual(response.data['kyc']['address'], '456 Elm St')
    ...
    def test_update_kyc_view_no_permission(self):
        ...
        # Attempt to update KYC for the original user
    ...
    def test_create_new_kyc_record_if_none_exists(self):
        # Remove any existing KYC record for the user
        KYC.objects.filter(user=self.user).delete()

        # Ensure no KYC record exists for the user
        self.assertFalse(KYC.objects.filter(user=self.user).exists())

        # Send POST request to update KYC
        response = self.client.post(reverse('update-kyc'), self.kyc_data)

        # Check if the response status is 200 OK
        self.assertEqual(response.status_code, 200)

        # Verify that a new KYC record is created
        self.assertTrue(KYC.objects.filter(user=self.user).exists())

        # Check that the KYC data in the response matches the input data
        kyc_record = KYC.objects.get(user=self.user)
        self.assertEqual(kyc_record.first_name, self.kyc_data['first_name'])
        self.assertEqual(kyc_record.last_name, self.kyc_data['last_name'])
        self.assertEqual(kyc_record.address, self.kyc_data['address'])
    ...
    def test_update_kyc_success(self):
        ...
        url = reverse('update-kyc')  # Assuming the URL pattern name is 'update-kyc'
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['message'], 'KYC updated successfully')
        self.assertEqual(response.data['kyc']['first_name'], valid_kyc_data['first_name'])
        self.assertEqual(response.data['kyc']['last_name'], valid_kyc_data['last_name'])
        self.assertEqual(response.data['kyc']['address'], valid_kyc_data['address'])
